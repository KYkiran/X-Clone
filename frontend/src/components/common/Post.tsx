	...
	const { mutate: likePost, isPending: isLiking } = useMutation({
		mutationFn: async (): Promise<string[]> => {
			try {
				...
				return data; // Your API returns the likes array directly
			} catch (error) {
				throw new Error(error instanceof Error ? error.message : "Unknown error");
			}
		},
		...
		mutationFn: async (): Promise<Comment> => {
			try {
				const res = await fetch(`/api/posts/comment/${post._id}`, {
					...
					body: JSON.stringify({ text: comment } as CommentRequest),
				});
				const data = await res.json();
			...
			<div className='flex gap-2 items-start p-4 border-b border-gray-700'>
				<div className='avatar'>
					<Link to={`/profile/${postOwner.username}`} className='w-8 rounded-full overflow-hidden'>
						<img src={postOwner.profileImg || "/avatar-placeholder.png"} alt={`${postOwner.fullName}'s avatar`} />
					</Link>
				</div>
						...
						<div className='flex gap-4 items-center w-2/3 justify-between'>
							...
							<dialog id={`comments_modal${post._id}`} className='modal border-none outline-none'>
								...
								<form method='dialog' className='modal-backdrop'>
									<button className='outline-none' type="button">close</button>
								</form>
							</dialog>
